...
import {
  ...
  ChevronRight, Bed, CreditCard, UserCheck
} from 'lucide-react'
...
export default function AdminBookingsPage() {
  ...
  const handleStatusChange = async (id: string, status: string) => {
    try {
      ...
      const res = await fetchApi(`/api/admin/bookings/${id}/status`, {
        method: 'PUT',
  ...
  const getStatusVariant = (status: string) => {
    switch (status) {
      case 'approved': case 'paid': case 'checked_in': case 'completed': return 'success'
  ...
  const filtered = bookings.filter(b => {
    const matchesSearch = search === '' ||
      b.students?.full_name?.toLowerCase().includes(search.toLowerCase()) ||
      b.students?.user?.index_number?.toLowerCase().includes(search.toLowerCase())
    ...
    <div className="max-w-[1600px] mx-auto space-y-8">
      ...
      <div className="page-header bg-white p-8 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-[#003366] tracking-tight">Booking <span className="text-[#1A5F9E]">Ledger</span></h1>
          <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Active room allocations and lifecycle tracking</p>
        </div>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <AnimatedStatCard icon={ClipboardList} label="Total Bookings" value={bookings.length} iconColor="blue" />
        <AnimatedStatCard icon={Clock} label="Pending" value={countByStatus('pending')} iconColor="yellow" />
        <AnimatedStatCard icon={CreditCard} label="Paid" value={countByStatus('paid')} iconColor="green" />
        <AnimatedStatCard icon={UserCheck} label="Checked In" value={countByStatus('checked_in')} iconColor="purple" />
      </div>
          ...
          <Card className="border-none shadow-sm rounded-2xl bg-white overflow-hidden p-0">
            ...
            <div className="divide-y divide-slate-50">
              {filtered.length > 0 ? (
                filtered.map(b => (
                  <div
                    ...
                    className={`flex items-center justify-between p-6 cursor-pointer transition-all hover:bg-slate-50/50 ${
                      selectedBooking?.id === b.id ? 'bg-[#E8F1F8]/30 border-l-4 border-[#003366]' : ''
                    }`}
                  >
                    <div className="flex items-center gap-4 min-w-0">
                      <div className="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold border border-blue-200">
                        {b.students?.full_name?.[0] || 'S'}
                      </div>
                      <div>
                        <div className="font-bold text-slate-900">
                          {b.students?.full_name || 'Unknown Student'}
                        </div>
                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                          {b.students?.user?.index_number} • Rm {b.rooms?.room_number || '—'}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 shrink-0">
                      <ModernBadge variant={getStatusVariant(b.status)}>
                        {b.status?.replace('_', ' ')}
                      </ModernBadge>
                ...
                <div className="py-20">
                  <EmptyState icon={ClipboardList} title="No Bookings" description="No booking records match the current filters." />
                </div>
              )}
            </div>
          </Card>
          ...
          {selectedBooking ? (
            <Card className="border-none shadow-sm rounded-2xl bg-white overflow-hidden p-0 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="p-8 bg-gradient-to-br from-slate-50 to-white border-b border-slate-50">
                <div className="h-16 w-16 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-xl border border-blue-200 mb-6">
                  {selectedBooking.students?.full_name?.[0] || 'S'}
                </div>
                <div>
                  <h3 className="text-xl font-black text-slate-900 tracking-tight">
                  {selectedBooking.students?.full_name || 'Unknown Student'}
                  </h3>
                </div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{selectedBooking.students?.user?.index_number}</p>
              </div>

              <div className="p-8 space-y-6">
                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Status</span>
                  <ModernBadge variant={getStatusVariant(selectedBooking.status)}>
                    {selectedBooking.status?.replace('_', ' ')}
                  </ModernBadge>
                </div>
                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Room</span>
                  <span className="text-xs font-black text-slate-700">{selectedBooking.rooms?.room_number || 'N/A'}</span>
                </div>
                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl">
                ...
                <div className="border-t border-slate-100 pt-6 space-y-3">
                  ...
                  {selectedBooking.status === 'approved' && (
                    <Button
                      ...
                      disabled={actionLoading}
                      className="w-full bg-[#003366] hover:bg-[#1A5F9E] text-white rounded-2xl h-12 font-black text-xs uppercase tracking-widest shadow-lg shadow-[#003366]/20"
                    >
                      <UserCheck className="w-4 h-4 mr-2" /> Mark Check-In
                    </Button>
                  )}
                  {selectedBooking.status !== 'cancelled' && selectedBooking.status !== 'completed' && selectedBooking.status !== 'checked_out' && (
                    <Button
            ...
            <Card className="border-none shadow-sm rounded-2xl bg-[#003366] overflow-hidden p-8 text-white min-h-[400px] flex flex-col justify-center text-center relative">
              ...
              <div className="relative z-10 space-y-4 max-w-xs mx-auto">
                ...
                <h3 className="text-2xl font-black">Booking Inspector</h3>
                <p className="text-blue-200 text-xs font-bold uppercase tracking-widest leading-loose">
                  Select a booking from the list to view details, approve, or cancel.
                </p>
              </div>
            </Card>
          )}
