...
if (process.env.REDIS_URL || (process.env.REDIS_HOST && process.env.REDIS_PORT)) {
  ...
  redis = {
    ...
    setex: async (key: string, _ttl: number, value: string) => { mockStore.set(key, value); return 'OK'; },
...
export async function validateSession(sessionId: string): Promise<SessionData | null> {
  ...
  try {
    ...
    // Upstash Redis automatically parses JSON, while standard Redis returns a string
